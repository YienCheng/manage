...
export default {
  ...
  mounted () {
    ...
    if (sessionStorage.getItem('update_article_editor')) {
      this.form = JSON.parse(sessionStorage.getItem('updatearticle_editor'))
    }
  },
  components: {
    MarkdownEditor
  ...
  methods: {
    CtrlS (event) {
      if (event.keyCode === 83 && (navigator.platform.match('Mac') ? event.metaKey : event.ctrlKey)) {
        ...
        sessionStorage.setItem('update_article_editor', JSON.stringify(this.form))
    ...
    saveArticle (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          let params = Object.assign({id: this.$route.params.id}, this.form)
  ...
  destroyed () {
    sessionStorage.removeItem('update_article_editor')
